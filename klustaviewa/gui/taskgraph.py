...
class TaskGraph(AbstractTaskGraph):
    ...
    def _select(
        ...
        self.tasks.selection_task.select(
            clusters,
            wizard,
        )
    ...
    def correlograms_computed_callback(
        self, clusters, correlograms, ncorrbins, corrbin, sample_rate, wizard
    ):
        ...
        self.correlograms_computed(
            clusters, correlograms, ncorrbins, corrbin, sample_rate, wizard
        )
    ...
    def similarity_matrix_computed_callback(
        ...
        self.similarity_matrix_computed(
            clusters_selected, matrix, clusters, cluster_groups, target_next=target_next
        )
    ...
    def _compute_correlograms(self, clusters_selected, wizard=None):
        ...
        spiketimes_excerpts = get_excerpts(
            spiketimes, nexcerpts=nexcerpts, excerpt_size=excerpt_size
        )
        clusters_excerpts = get_excerpts(
            clusters, nexcerpts=nexcerpts, excerpt_size=excerpt_size
        )
    ...
    def _recluster(self):
        ...
        self.tasks.recluster_task.recluster(
            exp, channel_group=channel_group, clusters=clusters_selected
        )

    def _recluster_done(
        self, channel_group=0, clusters=None, spikes=None, clu=None, wizard=False
    ):
    ...
    def _compute_similarity_matrix(self, target_next=None):
        ...
        clusters_to_update = self.statscache.similarity_matrix.not_in_key_indices(
            clusters_all
        )
    ...
    def _update_cluster_view(self, clusters=None):
        ...
        object."""
        data = vd.get_clusterview_data(
            self.experiment, self.statscache, channel_group=self.loader.shank
        )
...
def after_merge(output):
    if output.get("wizard", False):
        r = [
            ("_invalidate", (output["clusters_to_merge"],)),
            ...
            (
                "_wizard_show_pair",
                ((output["cluster_merged"], output["cluster_merged_colors"][0]),),
            ),
        ]
    else:
        r = [
...
def after_merge_undo(output):
    clusters_to_invalidate = union(
        output["clusters_to_merge"], [output["cluster_merged"]]
    )
    if output.get("wizard", False):
        r = [
            ("_invalidate", (clusters_to_invalidate,)),
            ("_compute_similarity_matrix", ()),
...
def after_split(output):
    clusters_to_update = sorted(
        set(output["clusters_to_split"]).union(set(output["clusters_split"]))
        - set(output["clusters_empty"])
    )
    if output.get("wizard", False):
        r = [
            ("_invalidate", (output["clusters_to_split"],)),
...
def after_split_undo(output):
    clusters_to_invalidate = union(
        output["clusters_to_split"], output["clusters_split"]
    )
    if output.get("wizard", False):
        r = [
            ("_invalidate", (clusters_to_invalidate,)),
